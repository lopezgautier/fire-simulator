    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-8 flex flex-col gap-6">
        ...
        <div className="flex items-start justify-between pt-2">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">FIRE Simulator</h1>
            <p className="text-gray-400 mt-1 text-sm">
              Accumulation → decumulation · CHF · Swiss 2nd pillar
            </p>
          </div>
          <button
            onClick={handleShare}
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-gray-200 bg-white text-xs font-medium text-gray-500 hover:bg-gray-50 transition-colors"
          >
            {copied ? '✓ Copied' : 'Share'}
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Portfolio Overview</h2>
            <p className="text-xs text-gray-400">Phase 2 settings applied</p>
          </div>
          <PortfolioChart
        ...
        <div className="flex items-center justify-between border-b border-gray-200">
          ...
          <button
            ...
            className="text-xs font-medium text-gray-500 hover:text-gray-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors pb-3"
          >
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className={`px-4 pb-3 pt-0.5 text-sm font-medium border-b-2 transition-colors ${
        active
